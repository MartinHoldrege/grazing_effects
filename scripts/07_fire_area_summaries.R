...
# doesn't  make sense to compare different grazing scenarios
# 
ba_gcm_delta_graze1 <- ba_gcm2 %>% 
  ...
  mutate(ba_delta = ba_ref - ba,
         ba_delta_perc = round(ba_delta/ba_ref*100, 1),
         ba_delta = round(ba_delta))
...
ba_smry_delta_graze1 <- ba_gcm_delta_graze1 %>% 
  ...
  summarize(across(.cols = c('ba_delta', 'ba_delta_perc'),
                   .fns = list(
                     'median' = median,
                     'low' = calc_low,
                     'high' = calc_high
...
write_csv(ba_smry_delta_graze1,
